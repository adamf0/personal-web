...
function ImageCarousel({ images = [], category = "Web" }) {
  ...
  return (
    <div
      className={`relative ${category == "Mobile" ? "max-w-[360px]" : "w-full"} ${category == "Mobile" ? "max-h-[640px]" : "max-h-[600px]"} overflow-hidden rounded-lg shadow-md`}
    >
      {/* Gambar */}
      <div
...
export default function PortfolioDetail() {
  ...
  const listArch = (detail.History ?? []).find(
    (history) =>
        ...
        <div key={uuid()}>
            ...
            <div>
              <p className="font-medium text-gray-600 mb-2">
                ...
                {history.position != "current" && (
                  <span className="bg-green-400 rounded text-[11px] px-4 py-1">
                    {history.position}
                  </span>
                )}
              </p>
          ...
          {history?.link && (
            <div key={uuid()}>
              ...
              <ol>
                {history.link.map((link) => (
                  <li>
                    <Link
                      to={link}
                      className="text-sm font-bold text-green-600 hover:text-green-500 text-center mb-6"
                    >
                      {link}
                    </Link>
                  </li>
          ...
          {images.length > 0
            ? images.map((image) => {
                return (
                  <div
                    key={uuid()}
                    className="bg-gray-100 h-48 flex items-center justify-center rounded shadow"
                  >
        ...
        <section className="relative min-h-screen bg-gray-50 overflow-hidden">
          ...
          <div className="relative z-10 px-6 lg:px-24 py-20 max-w-7xl mx-auto space-y-16">
            ...
            <section>
              ...
              <div
                className={`${data?.category === "Mobile" && "flex justify-center"}`}
              >
            ...
            <section className="py-6 px-6 md:px-24 text-center">
              {/* Deskripsi */}
              <div className="mb-8">
                <p className="text-gray-600">{data?.description ?? NotFound}</p>
                {renderSmallInfo()}
              </div>
